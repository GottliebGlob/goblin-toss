...
const BetSection = ({ choice }: BettingSectionProps) => {
    ...
    const getFlipText = () => {
        return (
            chain?.id !== CoinFlip.chainId
            ? "Change Network"
            : Number(amount) <= 0 ? "Select amount to flip" : isLoading
            ? "Flipping..."
            : "Flip Heads"
        )
    }
    ...
    return (
      <div className="w-[300px] md:w-[400px] lg:w-[450px]">
        <div className="flex justify-between bg-[#FBC13C] rounded-xl w-full h-[56px] mt-4">
          ...
          <div onClick={() => setAmount(toEther(balance?.value))} 
        ...
        <div className="flex flex-col w-full text-white text-[12px] gap-y-2 mt-4">
          <div className="flex justify-between items-center">
            ...
            <p>Target payout: ~{(Number(amount) * 1.94).toString().substring(0,6)} ETH</p>
          </div>
